...
export class IncomeCategory {
    ...
    actionsButtons(): void {
        ...
        this.categoriesContainer.addEventListener('click', (e: MouseEvent): void => {
            ...
            if (target.id === 'add-category') {
                e.preventDefault();
                void this.openNewRoute('/income-category-creation');
                return;
            }
            ...
            if (target.classList.contains('edit')) {
                ...
                if (id) {
                    void this.openNewRoute(`/income-category-edit?id=${encodeURIComponent(id)}`);
                }
                return;
            }
    ...
    async loadCategories(): Promise<void> {
        try {
            const result: HttpUtilsResult<CategoryObject[]> = await HttpUtils.request<CategoryObject[]>('/categories/income', 'GET');
    ...
    displayCategories(categories: CategoryObject[]): void {
        ...
        categories.forEach((cat: CategoryObject): void => {
            ...
            <div class="card card-outline shadow-none border border-gray-300 d-flex flex-column justify-content-center p-3 rounded-12">
                ...
                <div>
                    <a href="/income-category-edit?id=${encodeURIComponent(cat.id)}" data-id="${cat.id}" 
                    class="edit btn btn-primary pt-2 pb-2 pl-3 pr-3 rounded font-weight-medium">Редактировать</a>
                    <button type="button" class="delete btn btn-danger ml-2 pt-2 pb-2 pl-3 pr-3 rounded font-weight-medium" 
                    data-toggle="modal" data-target="#myModal" data-id="${cat.id}">Удалить</button>
                </div>
            </div>`;
            if (this.categoriesContainer) {
    ...
    async deleteCategory(id: string | number): Promise<void> {
        try {
            const result: HttpUtilsResult = await HttpUtils.request<unknown>(`/categories/income/${id}`, 'DELETE');
